...
function DailyTasksPanel({
  ...
  return (
    <View style={styles.panel}>
      <View style={styles.panelHeader}>
        <View>
          <Text style={styles.panelTitle}>Daily tasks</Text>
          <Text style={styles.muted}>
            {completedCount}/{tasks.length} done today • {completionRate}%
          </Text>
        </View>
        <View style={styles.taskRing}>
          <Text style={styles.taskRingText}>{completionRate}%</Text>
        </View>
      </View>

      <View style={styles.taskAddBox}>
        <TextInput
          value={newTaskTitle}
        ...
        <View style={styles.row}>
          <TextInput
            ...
            placeholderTextColor="#8F8AA9"
            style={[styles.input, styles.taskCategoryInput]}
          />
          <Pressable style={styles.addTaskButton} onPress={onAdd}>
      ...
      {tasks.map((task) => {
        ...
        const streak = countDateStreak(task.completedDates);
        ...
        return (
          <View key={task.id} style={styles.taskRow}>
            ...
            <Pressable style={styles.taskMain} onPress={() => onToggle(task.id)}>
              ...
              <Text style={styles.muted}>
                {task.category} • {streak} day streak
              </Text>
            </Pressable>
            <Pressable style={styles.taskArchiveButton} onPress={() => onArchive(task.id)}>
              <Text style={styles.taskArchiveText}>Hide</Text>
            </Pressable>
          </View>
        );
      })}
      {tasks.length === 0 ? <Text style={styles.muted}>Add a daily task to start tracking.</Text> : null}
    </View>
  );
}
